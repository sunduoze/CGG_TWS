...
#define BLE_MAC_ADDR1 "0xCA982C8DEB3C"
#define BLE_MAC_ADDR2 "0x60C0BF29DA01"
/*********************************************************************
 * EXTERNAL VARIABLES
 */
...
void SimpleBLEObserver_Init( uint8 task_id )
{
  ...
  SerialPrintString("SimpleBLEObserver_SerialPrint Start init.\r\n");//enzo modify
...
static void simpleBLEObserver_HandleKeys( uint8 shift, uint8 keys)
{
  (void)shift;  // Intentionally unreferenced parameter
   /* if ( !simpleBLEScanning )
    {
      LCD_WRITE_STRING( "         66666", HAL_LCD_LINE_6 );
    }
    else
    {
      LCD_WRITE_STRING( "         77777", HAL_LCD_LINE_6 );
    }*/
  if ( keys & HAL_KEY_UP )
  {
    ...
    case GAP_DEVICE_INFO_EVENT:
      {
        ...
        //enzo debug
        if(pEvent->deviceInfo.rssi + 100 > 40)// > -40db
        {
          if(strncmp((const char *)BLE_MAC_ADDR1,bdAddr2Str( pEvent->deviceInfo.addr ), 14) == 0||
             strncmp((const char *)BLE_MAC_ADDR2,bdAddr2Str( pEvent->deviceInfo.addr ), 14) == 0||
             strncmp((const char *)ble_mac_addr_g,bdAddr2Str( pEvent->deviceInfo.addr ), 14) == 0  )
          {
            LCD_WRITE_STRING( bdAddr2Str( pEvent->deviceInfo.addr ),
                          HAL_LCD_LINE_3 );
            LCD_WRITE_STRING_VALUE("rssi",(uint16)(pEvent->deviceInfo.rssi + 100),10 ,HAL_LCD_LINE_4);
            
            beacon_rssi_g = (uint16)(pEvent->deviceInfo.rssi + 100);
            ext_print_uart_str("scaned\r\n");
          }
        }
      }
      break;
    ...
    case GAP_DEVICE_DISCOVERY_EVENT:
      {
        ...
        //osal_memcpy(simpleBLEDevInfoList, pEvent->deviceInfo.rssi,
        //            (sizeof( gapDevRec_t ) * pEvent->discCmpl.numDevs))
